...
import { clientShiftSignOffData, } from "../../StaffBooking.utils";
...
const CompletedConfirmModal = (props: any) => {
  const { isCompletedConfirmModal, setIsCompletedConfirmModal, signedOffDetails } = props;
  ...
  const handleCompletedConfirmShift = async () => {
    const { data: completed } = await createConfirm({
      staffShiftId: signedOffDetails._id,
      modifyReason: shiftInformation.modifyReason,
      modifiedStart: `${dayjs(signedOffDetails.shift.shiftDate).format('YYYY-MM-DD')}T${shiftInformation.checkInTime}`,
      modifiedEnd: `${dayjs(signedOffDetails.shift.shiftDate).format('YYYY-MM-DD')}T${shiftInformation.checkOutTime}`,
      shiftStatus: 'SIGNEDOFF',

    });
    const { data } = await addRating({
      carerId: signedOffDetails?.carer._id,
      shiftId: signedOffDetails._id,
          ...
          <Row gutter={[20, 20]} align="middle" justify="center">
            <Col xl={8} lg={8} md={24} sm={24} xs={24}>
              <div className="profile-information text-center">
                <img src={`https://rnd-s3-public-dev-001.s3.eu-west-2.amazonaws.com/${signedOffDetails?.carer?.profilePhoto?.mediaId}.${signedOffDetails?.carer?.profilePhoto?.mediaMeta?.extension}`} alt="" />
                <h2 className="fs-16 fw-500 m-0 title-color">{`${signedOffDetails?.carer?.firstName} ${signedOffDetails?.carer?.lastName}`}</h2>
